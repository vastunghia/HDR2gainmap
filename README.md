...
- **Parallel processing** with a **progress bar**, per‑file **structured logging**, and an end‑of‑run **summary** (written / skipped / failed).
...
If a potential issue is detected (e.g., encoder differences on some macOS configurations), a warning is emitted.
You can skip verification with `--do_not_verify`.

For manual inspection you can also use Adobe’s **Gain Map Demo App for macOS**:  
<https://www.adobe.com/go/gainmap_demoapp_mac>

If the gain map is missing:
